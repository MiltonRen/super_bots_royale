...
async function buildAndAutoReload() {
  ...
  // Auto-reload uses an HTTP server as an even stream to reload the browser
  http
  ...
  console.log("[auto-reload] initial build succeeded!")
    ...
    .on("all", async (event, path) => {
      ...
      if (path.includes("javascript")) {
        try {
          await setTimeout(20)
          await context.rebuild()
          console.log("[auto-reload] build succeeded!")
        } catch (error) {
          console.error("[auto-reload] build failed!", error)
        }
      }
